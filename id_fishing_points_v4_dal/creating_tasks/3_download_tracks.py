...
with open("toadd_"+today_date+".csv", "rU") as csvfile:
    reader = csv.DictReader(csvfile)
    for row in reader:
        vessels.append(row['mmsi']) 
        mmsi_types[row['mmsi']] = row['shiptype']
...
with gzip.open(path_to_table_dir+table+".zip", 'rb') as f:
    ...
    for row in reader:
        ...
        if len(mmsi)>0 and mmsi_year_month not in mmsi_year_months:
            ...
            if len(lats) and fast_speeds>40: #has to have at least 100 positions in the month
                js = {}
                js['lats']=lats
                js['lons']=lons
                js['sogs']=sogs
                ...
                f.write(t)
                f.close()
                thecommand = "gsutil -m cp "+ json_path + json_filename +" gs://gfw-crowd/"+json_filename+\
                    ";gsutil -m acl set -R -a public-read gs://gfw-crowd/"+json_filename
                print thecommand

                # upload to gcs and set the permissions to be public
                os.system(thecommand)
                # add to the tasks
                tasks.append((mmsi,2015,m,mmsi_types[str(mmsi)]))
